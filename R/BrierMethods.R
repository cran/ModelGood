...
Brier.default <- function(object,y,crRatio=1,...){
  N <- length(object)
      ...
      compute.BootcvBSList <- lapply(1:B,function(b){
        ...
        if (is.null(fit.b)){
          ...
          if (inherits(try2predict,"try-error")==TRUE){
            ...
            failed <- NA
            innerBootcvBS <- Brier.default(object=pred.b,y=Y[vindex.b],crRatio=crRatio)
          }
        }
...
# }}}
  # {{{ averaging the B bootcv BS curves
